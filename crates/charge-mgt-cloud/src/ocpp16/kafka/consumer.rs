...
use crate::ocpp16::kafka::dispatcher::MessageDispatcher;
...
pub async fn spawn_kafka_consumer(state: AppState) -> anyhow::Result<()> {
    ...
    let consumer: StreamConsumer = ClientConfig::new()
        .set("group.id", config.kafka.consumer_group.as_str())
    ...
    let dispatcher = MessageDispatcher::new(state.clone());
    let pattern = format!("{}.req.", config.kafka.topic_prefix);
    let re_discover = config.kafka.req_topics.is_empty();
...
fn resolve_req_topics(state: &AppState, consumer: &StreamConsumer) -> Vec<String> {
    if let Ok(config) = state.config() {
        let topics = config.kafka.req_topics.clone();
        if !topics.is_empty() {
