...
use ocpp_1_6::confs::{
    ...
    SendLocalListConfirmation, SetChargingProfileConfirmation,
};

impl Handler<SetChargingProfileConfirmation> for SetChargingProfileRequest {
    #[cfg(feature = "cs_send_message_by_http")]
    async fn http_handler(state: &AppState, msg: &CloudMessage) -> Result<SetChargingProfileConfirmation, HandlerError> {
        todo!()
    }

    #[cfg(feature = "cs_send_message_by_mq")]
    async fn mq_handler(state: &AppState, msg: &CloudMessage) -> Result<SetChargingProfileConfirmation, HandlerError> {
        todo!()
    }
}
