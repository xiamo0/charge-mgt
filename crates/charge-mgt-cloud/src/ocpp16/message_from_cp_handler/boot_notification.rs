...
use crate::ocpp16::entity::charge_point::{ActiveModel, Column as ChargePointColumn, Entity as ChargePoints};
    ...
    async fn handel_detail(
        ...
        if req.charge_point_vendor.len() > 20 {
            return Err(AppError::OCPP_1_6_ERROR {
                ...
                detail: format!("请求ID {}, chargePointVendor must be <= 20 characters", msg.unique_id),
            });
        }
        if req.charge_point_model.len() > 20 {
            return Err(AppError::OCPP_1_6_ERROR {
                action: ACTION_BOOT_NOTIFICATION_CONFIRMATION.into(),
                detail: format!("请求ID {}, chargePointModel must be <= 20 characters", msg.unique_id),
            });
        }

        // 必须先有 serial 才能匹配预注册记录（boot 流程要求桩预注册过）
        let charge_point_serial_number = req.charge_point_serial_number.clone().ok_or_else(|| {
            AppError::OCPP_1_6_ERROR {
                action: ACTION_BOOT_NOTIFICATION_CONFIRMATION.into(),
                detail: format!("请求ID {}, charge_point_serial_number must not null", msg.unique_id),
            }
        })?;
        let charge_box_serial_number = req.charge_box_serial_number.clone().ok_or_else(|| {
            AppError::OCPP_1_6_ERROR {
                action: ACTION_BOOT_NOTIFICATION_CONFIRMATION.into(),
                detail: format!("请求ID {}, charge_box_serial_number must not null", msg.unique_id),
            }
        })?;
        ...
        let existing = ChargePoints::find()
            ...
            .filter(ChargePointColumn::ChargePointSerialNumber.eq(charge_point_serial_number.clone()))
