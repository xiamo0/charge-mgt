...
impl IncomingEnvelope {
    pub fn parse(text: &str) -> Result<Self, EnvelopeError> {
        let v: Value = serde_json::from_str(text)
            .map_err(|e| EnvelopeError(format!("not JSON: {e}")))?;
        let arr = v.as_array().ok_or_else(|| EnvelopeError("not a JSON array".into()))?;
        let type_id = arr
            .first()
            .and_then(|x| x.as_i64())
        ...
        match type_id {
            2 => {
                ...
                let uid = arr[1].as_str().ok_or_else(|| EnvelopeError("uid not a string".into()))?.to_string();
                let action = arr[2].as_str().ok_or_else(|| EnvelopeError("action not a string".into()))?.to_string();
            ...
            4 => {
                ...
                let uid = arr[1].as_str().ok_or_else(|| EnvelopeError("uid not a string".into()))?.to_string();
                let code = arr[2].as_str().ok_or_else(|| EnvelopeError("error_code not a string".into()))?.to_string();
                let description = arr[3].as_str().ok_or_else(|| EnvelopeError("error_description not a string".into()))?.to_string();
...
pub fn parse_raw_header(text: &str) -> Result<(i64, String, Option<String>), EnvelopeError> {
    let v: Value = serde_json::from_str(text)
        .map_err(|e| EnvelopeError(format!("not JSON: {e}")))?;
    let arr = v.as_array().ok_or_else(|| EnvelopeError("not a JSON array".into()))?;
    let type_id = arr.first().and_then(|x| x.as_i64())
        .ok_or_else(|| EnvelopeError("missing message_type_id".into()))?;
    let uid = arr.get(1).and_then(|x| x.as_str())
        .ok_or_else(|| EnvelopeError("missing/invalid uid".into()))?.to_string();
